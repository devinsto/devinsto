...
<template>
    ...
    <h3 class="mb-8 text-3xl text-gray-900 dark:text-white">Devinsto
        Apprenez, Créez, Collaborez sans limites.</h3>
    ...
    <div class="flex flex-col gap-8 border-l-2 border-l-green-100 md:flex-row">
        <!-- Navigation tabs -->
        <div class="flex gap-1 overflow-x-auto pb-4 md:flex-col md:pb-0" :class="{
            'flex-row': isMobile,
            'w-64 flex-col': !isMobile,
        }">
            <button v-for="tab in tabs" :key="tab.id" @click="activeTab = tab.id as TabType"
                class="rounded-lg px-4 py-3 text-left transition-colors" :class="{
        ...
        <div class="min-h-[400px] flex-1 rounded-xl bg-gray-50 p-6 dark:bg-gray-800">
            ...
            <div v-if="activeTab === 'logs'" class="space-y-4">
                <div v-for="(log, index) in content as LogEntry[]" :key="index"
                    ...
                    <div class="flex items-start gap-4">
                        ...
                        <div class="flex-1">
                            ...
                            <div v-if="log.message" class="mt-1 text-sm" :class="{
                            ...
                            <div v-if="log.error" class="mt-1 text-sm text-red-600 dark:text-red-400">{{ log.error }}
                            </div>
                            <div v-if="log.details" class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ log.details
                                }}</div>
                        </div>
                        <div v-if="log.status" class="rounded-full px-2 py-1 text-xs" :class="{
            ...
            <div v-if="activeTab === 'commands'" class="space-y-4">
                <div v-for="(cmd, index) in content as CommandEntry[]" :key="index"
                    class="rounded-lg bg-white p-4 shadow-sm dark:bg-gray-700">
                    <div class="flex items-start gap-4">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="mt-2 text-sm"><span class="text-gray-500 dark:text-gray-400">source:</span> {{
                                cmd.source }}</div>
            ...
            <div v-if="activeTab === 'metrics'" class="grid grid-cols-1 gap-4 md:grid-cols-3">
                <div v-for="(metric, index) in content as MetricEntry[]" :key="index"
                    class="rounded-lg bg-white p-4 shadow-sm dark:bg-gray-700">
                    <div class="flex items-start justify-between">
                        ...
                        <div class="flex h-8 w-8 items-center justify-center rounded-full" :class="{
                            ...
                            <svg v-if="metric.trend === 'up'" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                                viewBox="0 0 20 20" fill="currentColor">
                                <path fill-rule="evenodd"
                                    d="M12 7a1 1 0 110-2h5a1 1 0 011 1v5a1 1 0 11-2 0V8.414l-4.293 4.293a1 1 0 01-1.414 0L8 10.414l-4.293 4.293a1 1 0 01-1.414-1.414l5-5a1 1 0 011.414 0L11 10.586 14.586 7H12z"
                                    clip-rule="evenodd" />
                            </svg>
                            <svg v-else-if="metric.trend === 'down'" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                                viewBox="0 0 20 20" fill="currentColor">
                                <path fill-rule="evenodd"
            ...
            <div v-if="activeTab === 'command-palette'" class="space-y-4">
                <div v-for="(item, index) in content as PaletteEntry[]" :key="index"
